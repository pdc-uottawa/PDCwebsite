...
const imageModal = require("../models/PictureModel");
    ...
    console.log("here1");
        if(!req.file) 
            return res.send('Please upload a file')
             ...
             res.send("uploaded successfully"); //redirect(redirectpath);
        }
    });
...
router.get('/getImages', (req, res) => {
  ...
  gfs.files.find().toArray((err, files) => {
    ...
    const f =files
    .map(file=>{
      if(file.contentType === 'image/jpeg' || file.contentType === 'image/png' || file.contentType === 'image/jpg' || file.contentType === 'image/svg+xml'){
        file.isImage = true;
      }
      else{
        file.isImage = false;
